...
RUN Rscript -e 'if (!require("pacman")) install.packages("pacman"); pacman::p_load(lubridate,rnoaa,ggplot2,moments,operators,topmodel,DEoptim,XML,data.table,RSQLite,argparse,stringi,stringr,sqldf,readr,rgeos,rgdal,sf,readr,tools,diffobj,png,grid,gridExtra,purrr,raster);p_install_gh("USGS-R/ncdfgeom"); system("svn checkout svn://scm.r-forge.r-project.org/svnroot/ecohydrology/"); install.packages(c("ecohydrology/pkg/EcoHydRology/","ecohydrology/pkg/SWATmodel/"),repos = NULL)' 
WORKDIR /mintswat
